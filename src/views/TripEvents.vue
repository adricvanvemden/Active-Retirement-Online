<template>
  <div class="event">
    <div class="header">
      <h1>Hello "firstname", look at al these <b>trip</b> related events!</h1>
    </div>
    <div class="infoBox" >
      <div class="eventInfo" v-for="event in events" :key="event.id">
        <div class="nameRegister" v-if="event.type === 'trip'">
          <div class="eventName">
            {{ event.eventName }}
          </div>
        ...
        <div class="eventDate" v-if="event.type === 'trip'">
          {{ event.date }}
        </div>
        <div class="eventTime" v-if="event.type === 'trip'">
          {{ event.startTime }} - {{ event.endTime }} @{{ event.location }}<br>
        </div>
        <div class="eventDescription" v-if="event.type === 'trip'">
          {{ event.description }}
        </div>
        <hr v-if="event.type === 'trip'">
      </div>
    </div>
  </div>
</template>
