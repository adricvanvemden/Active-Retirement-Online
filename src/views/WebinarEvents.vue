<template>
  <div class="event">
    <div class="header">
      <h1>Hello "firstname", look at al these <b>webinar</b> related events!</h1>
    </div>
    <div class="infoBox" >
      <div class="eventInfo" v-for="event in events" :key="event.id">
        <div class="nameRegister" v-if="event.type === 'webinar'">
          <div class="eventName">
            {{ event.eventName }}
          </div>
        ...
        <div class="eventDate" v-if="event.type === 'webinar'">
          {{ event.date }}
        </div>
        <div class="eventTime" v-if="event.type === 'webinar'">
          {{ event.startTime }} - {{ event.endTime }} @{{ event.location }}<br>
        </div>
        <div class="eventDescription" v-if="event.type === 'webinar'">
          {{ event.description }}
        </div>
        <hr v-if="event.type === 'webinar'">
      </div>
    </div>
  </div>
</template>
